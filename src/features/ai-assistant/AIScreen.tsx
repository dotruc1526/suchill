...
const AI_KNOWLEDGE_BASE: Record<string, string> = {
  ...
  'điện biên phủ': 'Chiến dịch Điện Biên Phủ (13/3–7/5/1954) do Đại tướng Võ Nguyên Giáp chỉ huy. Sau 56 ngày đêm chiến đấu kiên cường, ta tiêu diệt hoàn toàn tập đoàn cứ điểm của Pháp — chiến thắng lừng lẫy năm châu!',
  'võ nguyên giáp': 'Đại tướng Võ Nguyên Giáp (1911–2013) là Tổng Tư lệnh Quân đội Nhân dân Việt Nam, thiên tài quân sự thế kỷ XX nổi tiếng với chiến lược chiến tranh nhân dân.',
  'genève': 'Hiệp định Genève ký ngày 21/7/1954 quy định: đình chỉ chiến sự, lập vĩ tuyến 17 làm ranh giới quân sự tạm thời tại sông Bến Hải và quy định tổng tuyển cử thống nhất vào năm 1956.',
}
...
export function AIScreen() {
  ...
  const sendMessage = (text: string) => {
    ...
    setTimeout(() => {
      const key = Object.keys(AI_KNOWLEDGE_BASE).find(k => text.toLowerCase().includes(k))
      const reply = key
        ? AI_KNOWLEDGE_BASE[key]
        : 'Đây là một câu hỏi thú vị! Mình đang tìm kiếm trong nguồn tư liệu lịch sử. Bạn có thể hỏi cụ thể hơn về sự kiện 1954, Điện Biên Phủ hay Hiệp định Genève không? 📚'
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          {aiSuggestions.map(s => (
            <button
              ...
              className="shrink-0 px-3 py-1.5 rounded-full font-sans text-xs whitespace-nowrap transition-transform active:scale-95"
              style={{
